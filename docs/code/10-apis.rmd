...
From the output of `r`, we can see that the query was successful (`Status: 200`), the content is in `json` format, and its size is `219kB`.
...
Between these two dates, there were 1177 articles in the NYT mentioning "inequality".
...
nyt_years_count(q="inequality", yearinit=1980, yearend=2017)
```
...
```{r}
counts <- nyt_years_count(q="inequality", yearinit=1980, yearend=2017)
plot(1980:2017, counts, type="l", main="Mentions of inequality on the NYT, by year",
...
```{r}
counts <- nyt_dates_count(q="trump", init = as.Date("2015/01/01"), end = as.Date("2018/07/31"), by="month")
